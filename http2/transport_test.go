...
// BenchmarkClientConnEncodeHeaders measures request header encoding directly,
// without a round trip, in the two shapes that matter: with a Header-Order:
// key (ordered sort) and without one (lexicographic sort).
// TestClientConnEncodeHeadersOrder pins the exact header sequence
// encodeHeaders emits, across every combination of the two ordering keys
// being present or absent, so that changes to how the headers are prepared
// cannot alter what goes on the wire.
func TestClientConnEncodeHeadersOrder(t *testing.T) {
	tests := []struct {
		name          string
		pseudoOrder   []string // Transport.PseudoHeaderOrder
		header        http.Header
		method        string
		addGzipHeader bool
		trailers      string
		...
		{
			// No ordering keys on the request, so the regular headers sort
			// lexicographically and the Transport supplies the pseudo order.
			name:          "no order keys with transport pseudo order",
			pseudoOrder:   []string{":method", ":authority", ":scheme", ":path"},
			header:        http.Header{"Zeta": {"z"}, "Alpha": {"a"}, "Mid": {"m1", "m2"}},
			addGzipHeader: true,
			contentLength: -1,
			want: []string{
				":method: GET", ":authority: www.example.org", ":scheme: https", ":path: /p",
				"alpha: a", "mid: m1", "mid: m2", "zeta: z",
				"accept-encoding: gzip, deflate, br",
				"user-agent: " + defaultUserAgent,
			},
		},
		{
			// Pseudo order only: regular headers still sort lexicographically.
			name: "pheader order only",
			header: http.Header{
				"Zeta": {"z"}, "Alpha": {"a"},
			...
			contentLength: -1,
			want: []string{
				":method: GET", ":path: /p", ":authority: www.example.org", ":scheme: https",
				"alpha: a", "zeta: z",
				"user-agent: " + defaultUserAgent,
			},
		},
		{
			// Header order only, with keys both in and out of the list. Keys
			// absent from the list follow, lexicographically.
			name:        "header order only, some keys unlisted",
			pseudoOrder: []string{":method", ":authority", ":scheme", ":path"},
			header: http.Header{
				"In-Order": {"x"}, "Also-Order": {"y"},
				"Zeta": {"z"}, "Alpha": {"a"},
				http.HeaderOrderKey: {"in-order", "also-order"},
			},
			contentLength: -1,
			want: []string{
				":method: GET", ":authority: www.example.org", ":scheme: https", ":path: /p",
				"in-order: x", "also-order: y", "alpha: a", "zeta: z",
				"user-agent: " + defaultUserAgent,
			},
		},
		{
			name: "both order keys, with cookie splitting and content length",
			header: http.Header{
				"Cookie":             {"a=1; b=2;c=3"},
				"User-Agent":         {"ua"},
				"Content-Type":       {"application/json"},
				http.HeaderOrderKey:  {"user-agent", "cookie", "content-type"},
				http.PHeaderOrderKey: {":method", ":authority", ":scheme", ":path"},
			},
			method:        "POST",
			contentLength: 7,
			want: []string{
				":method: POST", ":authority: www.example.org", ":scheme: https", ":path: /p",
				"user-agent: ua", "cookie: a=1", "cookie: b=2", "cookie: c=3",
				"content-type: application/json",
				"content-length: 7",
			},
		},
		{
			// Connection-specific headers and Host are dropped, an empty
			// value slice is skipped, and a multi-value User-Agent is cut
			// to one - none of which depends on an ordering key.
			name:        "no order keys, filtered and skipped headers",
			pseudoOrder: []string{":method", ":authority", ":scheme", ":path"},
			header: http.Header{
				"Connection": {"keep-alive"}, "Upgrade": {"h2c"},
				"Transfer-Encoding": {"chunked"}, "Host": {"other.example"},
				"Skipped":    {},
				"User-Agent": {"first", "second"},
				"X-Kept":     {"yes"},
			},
			contentLength: -1,
			want: []string{
				":method: GET", ":authority: www.example.org", ":scheme: https", ":path: /p",
				"user-agent: first", "x-kept: yes",
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			u, err := url.Parse("https://www.example.org/p")
			if err != nil {
				t.Fatal(err)
			}
			method := tt.method
			...
			req := &http.Request{Method: method, URL: u, Header: tt.header}
			cc := &ClientConn{
				peerMaxHeaderListSize: 1 << 30,
				t:                     &Transport{PseudoHeaderOrder: tt.pseudoOrder},
			}
			cc.henc = hpack.NewEncoder(&cc.hbuf)

			cc.mu.Lock()
			hdrs, err := cc.encodeHeaders(req, tt.addGzipHeader, tt.trailers, tt.contentLength)
			cc.mu.Unlock()
			if err != nil {
			...
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("emitted headers:\n got: %q\nwant: %q", got, tt.want)
			}
		})
	}
}

func BenchmarkClientConnEncodeHeaders(b *testing.B) {
	run := func(b *testing.B, h http.Header) {
		b.ReportAllocs()
		u, err := url.Parse("https://www.example.org/some/path?q=1")
		if err != nil {
			b.Fatal(err)
		}
		req := &http.Request{Method: "GET", URL: u, Header: h}
		cc := &ClientConn{peerMaxHeaderListSize: 10 << 20, t: &Transport{}}
		cc.henc = hpack.NewEncoder(&cc.hbuf)
		cc.mu.Lock()
		defer cc.mu.Unlock()
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			if _, err := cc.encodeHeaders(req, true, "", -1); err != nil {
				b.Fatal(err)
			}
		}
	}
	...
	b.Run("Ordered", func(b *testing.B) {
		run(b, http.Header{
			...
			"accept-encoding":  {"gzip, deflate, br"},
			"referer":          {"https://www.example.org/x"},
			"cookie":           {"a=1; b=2; c=3"},
			http.HeaderOrderKey: {
				"sec-ch-ua", "accept", "x-requested-with", "sec-ch-ua-mobile",
				"user-agent", "content-type", "origin", "sec-fetch-site",
